...
const Success = () => {
    ...
    const fetchOrder = useCallback(async () => {
        try {
            ...
            // if (!response.data || typeof response.data !== 'object') throw new Error('بيانات غير صالحة');
            setOrder(response.data);
        } catch (err) {
            console.warn('⚠️ لم يتم جلب الفاتورة، سيتم عرض رسالة النجاح فقط.');
            setFetchFailed(true);
        } finally {
            setLoading(false);
        }
    ...
    // تتبع عملية الشراء فقط لو الفاتورة موجودة
    useEffect(() => {
        if (order) {
    ...
    const saveInvoiceAsImage = () => {
        ...
        html2canvas(invoiceElement, { scale: 2, logging: false, useCORS: true, allowTaint: true })
            .then(canvas => {
    ...
    return (
        <>
            ...
            <div className="success-page">
                ...
                {(fetchFailed) && (
                    <div className="invoice-success-only">
                        <div className="invoice-success-card">
                            <div className="invoice-success-icon">✅</div>
                            <h2 className="invoice-success-title">تم الدفع بنجاح</h2>
                            <p className="invoice-success-message">
                                شكراً لاختياركم متجرنا! تم إتمام عملية الدفع بنجاح.<br />
                                {/* إذا لم تظهر الفاتورة الآن، لا تقلق — فريق الدعم يمكنه تزويدك بها في أي وقت. */}
                            </p>
                            <a href="/" className="invoice-success-button">العودة للصفحة الرئيسية</a>
                        </div>
                    </div>
                )}
                            ...
                            <div className="invoice-header">
                                <div className="invoice-branding">
                                    ...
                                    <div className="company-info">
                                        ...
                                        <p className="company-contact"><FiPhone /> {order.customer?.phone || 'غير متوفر'}</p>
                                    </div>
                                </div>

                                <div className="invoice-meta">
                                    <div className="invoice-number"><p><FiHash /> رقم الفاتورة: {order.invoice_number || 'غير متوفر'}</p></div>
                                    <div className="invoice-date"><p><FiCalendar /> تاريخ الفاتورة: {formatDate(order.created_at)}</p></div>
                                </div>
                            </div>

                            <div className="invoice-body">
                                <div className="customer-section">
                                    <div className="section-title"><h3><FiMapPin /> معلومات العميل</h3></div>
                                ...
                                <div className="products-section">
                                    <div className="section-title"><h3><FiShoppingCart /> تفاصيل المنتجات</h3></div>
                                        ...
                                        <div className="table-body">
                                            {order.items.map((item, index) => (
                                                <div key={item.id ?? index} className="table-row">
                                                    ...
                                                    <div className="table-cell product-name">{item.product_title?.title || 'غير معروف'}</div>
                                                    {hasSize && <div className="table-cell">{item.variant_size?.name || 'غير محدد'}</div>}
                                                    {hasColor && <div className="table-cell">{item.variant_color?.name || 'غير محدد'}</div>}
                                                    <div className="table-cell">{item.quantity || 1}</div>
                                                    <div className="table-cell">{parseFloat(item.price?.amount || 0).toFixed(2)} {order.currency}</div>
                                                    <div className="table-cell">{(parseFloat(item.price?.amount || 0) * (item.quantity || 1)).toFixed(2)} {order.currency}</div>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                <div className="invoice-summary">
                                    <div className="summary-row"><span>المجموع الفرعي:</span><span>{subtotal.toFixed(2)} {order.currency}</span></div>
                                    {discount > 0 && <div className="summary-row discount-row"><span>الخصم:</span><span>-{discount.toFixed(2)} {order.currency}</span></div>}
                                    <div className="summary-row"><span>الشحن:</span><span>{shipping.toFixed(2)} {order.currency}</span></div>
                                    <div className="summary-row"><span>الضريبة:</span><span>{tax.toFixed(2)} {order.currency}</span></div>
                                    <div className="summary-row grand-total"><span>الإجمالي النهائي:</span><span>{total.toFixed(2)} {order.currency}</span></div>
                                </div>
                            </div>

                            <div className="invoice-footer">
                                <p className="thank-you">شكراً لاختياركم متجرنا</p>
                                <button onClick={saveInvoiceAsImage} className="download-btn"><FiSave /> حفظ الفاتورة كصورة</button>
                            </div>
                        </div>
                    </section>
                )}
